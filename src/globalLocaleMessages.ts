...
function _setLocale(
  ...
  locale: string,) {
  localeMessages.forEach(value =>
    (locale in value) && setLocalMessages(value[locale])
  );
}

export function getLocaleMessage<Extra extends Record<string, unknown> = any>(locale: string, key: string) {
