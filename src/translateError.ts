...
type Translate<M extends string = string, O extends Record<string, any> = {}> = (message: M, options?: O) => string;
...
export function translateError(error: ValidationError, translate: Translate<string, any>) {
...
export function buildFormatErrorTranslate(locale: string): Translate<string, any> {
  return (message, options) => {
    return Yup.ValidationError.formatError(getLocaleMessage(locale, message), options);
  }
}
